...
function makeTwo() {
	...
	try {
		eval('const e = "Новое значение";'); 
		resultTwo.textContent = 'Константа e была успешно перезаписана';
} catch(error){
...
function makeFive() {
	let inputData = -1;
	if (inputData < 0){
		throw new Error('Входные данные не могут быть отрицательными');
	}
		return inputData == 0 ? 1 : inputData * (inputData - 1);
}
document.querySelector('.b-5').addEventListener('click', () => {
	try {
	  const result = makeFive();
	  document.getElementById('result5').textContent = 'Результат:' + результат;
	} catch (error) {
	  console.error('Произошла ошибка:', error.message);
	}
  });
...
function makeSix() {
	...
	try {
		...
		return inputData== 0? 1: inputData * (inputData );
	  } catch (error) {
...
function makeEleven() {
	...
	if (phoneNumber.length < 10) {
		throw new Error ('Неверная длина номера телефона');
	  }
...
function makeSeventeen(str, func) {
	...
	try {
		...
		resultSeventeen.textContent = error.name;
	}
}
...
function makeEighteen() {
	...
	forWordFinally.textContent = 'finnaly'; // Отображение «наконец» в result18a
}
}
...
function makeNineteen() {
	...
	console.error(error); //Вывод ошибки в консоль
		...
		resultNineteen.textContent = 'finally19'; 
	}
}
...
function makeTwenty() {
		try {
		  ...
		  document.getElementById('result20').innerHTML = 'Error';
		} finally {
		  document.getElementById('result20a').innerHTML = 'finally20';
		}
	  }
...
function makeTwentyOne() {
	try {
	   ...
	   document.getElementById('result21').innerHTML = 'Error';
	 } finally {
	   document.getElementById('result21').innerHTML = 'finally21';
	 }
}
...
function makeTwentyThree() {
	...
	try {
		...
		console.log(user.name); //Не сработает
			...
			resultTwentyThree.textContent = 'Извините, в данных ошибка.';
		console.log(e.name);
		console.log(e.message);
	}
	}
...
function makeTwentyFour() {
	...
	let divisor = 0;
	if (divisor === 0) {
		throw new Error('Недопустимая операция: деление на ноль');
	}
	let result = 24 / divisor;
	return result;
} catch (error) {
    resultTwentyFour.textContent = 'К сожалению, при выполнении операции произошла ошибка.';
...
function makeTwentyFive() {
	...
	resultTwentyFive.textContent = 'К сожалению, при выполнении операции произошла ошибка.';
    console.log(error.name);
    console.log(error.message);
}
}
...
function makeTwentySix() {
    ...
    resultTwentySix.textContent = 'К сожалению, при асинхронном выполнении кода произошла ошибка.';
    console.log(error.name);
    console.log(error.message);
  }
}
